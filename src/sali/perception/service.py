...
human-readable ``detail``. Reused by the on-demand tool AND (later) the continuous perception loop.
"""
    ...
    async def snapshot(self, *, ui: bool = False) -> dict[str, Any]:
        ...
        if ui:
            node, detail = await atspi.ui_tree(
                self.s.system_python, max_depth=self.s.max_tree_depth, max_nodes=self.s.max_tree_nodes)
