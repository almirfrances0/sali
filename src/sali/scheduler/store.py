...
class ScheduleStore:
    def __init__(self, pool: Any, clock: Clock | None = None) -> None:
        self.pool = pool
        self.clock = clock or SystemClock()

    async def create(self, name: str, when: str, prompt: str) -> Schedule:
        ...
        next_at = cron.next_run(kind, spec, self.clock.now())
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "INSERT INTO schedule (name, kind, spec, prompt, next_run_at) "
                "VALUES ($1, $2, $3, $4, $5) RETURNING *",
                name, kind, spec, prompt, next_at)
    ...
    async def mark_fired(self, schedule_id: UUID, status: str) -> None:
        ...
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT kind, spec FROM schedule WHERE id = $1", schedule_id)
            if row is None:
                return
            next_at = cron.next_run(row["kind"], row["spec"], now)
            await conn.execute(
